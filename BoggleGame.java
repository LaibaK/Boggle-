...
public class BoggleGame {
    ...
    public void playRound(int size, String letters){
        ...
        // Change "wordlist.txt"
        Dictionary boggleDict = new Dictionary("/Users/laibakhan/CS207/khanlai6/Assignment1/wordlist.txt"); //you may have to change the path to the wordlist, depending on where you place it.
    ...
    private String randomizeLetters(int size) {
        ...
        if (size == 4) {
            ...
            // System.out.println(Arrays.toString(dice_small_grid));
            String line = "";
            // String fin = "";
            char c = 0;
            for (int i = 0; i < dice_small_grid.length; i++) {
                Random random = new Random();
                int index = random.nextInt(dice_small_grid[i].length());
                c = dice_small_grid[i].charAt(index);
                String s = String.valueOf(c);
                s.toLowerCase();
                line += s;
            }
            // fin = line.toLowerCase();
            // System.out.println(fin);
            return line;

        } else {
            Collections.shuffle(Arrays.asList(dice_big_grid));
            // System.out.println(Arrays.toString(dice_big_grid));
            String line = "";
            // String fin = "";
            char c = 0;
            for (int i = 0; i < dice_big_grid.length; i++) {
                Random random = new Random();
                int index = random.nextInt(dice_big_grid[i].length());
                c = dice_big_grid[i].charAt(index);
                String s = String.valueOf(c);
                line += s;
            }
            // fin = line.toLowerCase();
    ...
    private void findAllWords(Map<String,ArrayList<Position>> allWords, Dictionary boggleDict, BoggleGrid boggleGrid) {
        for (int i = 0; i < boggleGrid.numRows(); i++) {
            for (int j = 0; j < boggleGrid.numCols(); j++) {
                ArrayList<Position> empt = new ArrayList<Position>();
                //System.out.println("row"+ i);
                //System.out.println("col" + j);
                FindAllWords(i, j, boggleGrid, boggleDict, allWords, "", empt);
            }
        }
        //System.out.println(allWords.keySet());
    }

        private void FindAllWords(int row, int col, BoggleGrid boggleGrid, Dictionary boggleDict, Map<String, ArrayList<Position>> allWords, String soFar, ArrayList<Position> seen) {
            ...
            //System.out.println(visited);
            soFar += boggleGrid.getCharAt(row,col);
            //System.out.println(soFar);
            if (!boggleDict.isPrefix(soFar)) {
                //return if this is a dead end
                // System.out.println(soFar)
            ...
            if (boggleDict.containsWord(soFar) && (!allWords.containsKey(soFar)) && (soFar.length() > 3)) {
                // (!allWords.containsKey(soFar)) && soFar.length() > 3
                // RecordWordForPlayer(soFar, Computer);
                // Why are all lengthes not past 3?
                // System.out.println(soFar);
                allWords.put(soFar, seen);
            ...
            for (int i = 0; i < boggleGrid.numRows(); i++) {
                for (int j = 0; j < boggleGrid.numCols(); j++) {
                    //System.out.println("row"+ i);
                    //System.out.println("col" + j);
                    if (neighbors(row, col, i, j) && duplicated(i, j, seen)) {
                        //recur on the rest of the puzzle for all
                        // && NotDuplicated(i, j, visited))
                        FindAllWords(i, j, boggleGrid, boggleDict, allWords, soFar, seen);
        ...
        boolean neighbors(int row1, int col1, int row2, int col2) {
        /*
        int numO = row1 - row2;
        if (numO <= 0) {
            numO = -1 * numO;
        ...
        */
            if (row2 == -1 && col2 == -1) {
                return true;
            } else if (row1 - row2 > 1 || row2 - row1 > 1 || col1 - col2 > 1 || col2 - col1 > 1) {
                return false;
        ...
        boolean duplicated(int row, int col, ArrayList<Position> locations) {
            for (int i = 0; i < locations.size(); i++) {
                if (locations.get(i).getRRow() == row && locations.get(i).getCCol() == col) {
                    return false;
                }
            }
            return true;
    ...
    private void computerMove(Map<String,ArrayList<Position>> all_words){
        for (String key : all_words.keySet()) {
            if (!this.gameStats.getPlayerWords().contains(key)) {
                this.gameStats.addWord(key, BoggleStats.Player.Computer);
